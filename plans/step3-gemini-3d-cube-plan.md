...
## 1. Description
This plan outlines the integration of a 3D coordinate system and a controllable 3D cube using Three.js. The feature will be accessible via a new menu point, renaming the existing "menu5" to "Gemini3.1-pro - 3D cube".
...
- `src/gemini3.1-pro/features/threejs-cube/ThreeCubeScene.js`: Core logic for setting up Three.js engine, scene, camera, lights, coordinate system, and the cube itself.
...
- `src/gemini3.1-pro/features/threejs-cube/renderThreejsCubePage.js`: Responsible for rending the HTML structure (canvas, control menu) for Menu5.
...
1.  **Install Three.js**: Add `three` to package.json via npm.
2.  **Menu Integration**:
    - Update `src/components/Menu.js` to change the "menu5" entry to "Gemini3.1-pro - 3D cube".
    - Update `index.html` or routing logic to ensure clicking "Menu5" triggers the loading of the newly developed `renderThreejsCubePage.js`.

### Phase 2: Three.js Scene Setup (`ThreejsCubeScene.js`)
1.  **Canvas Setup**: Initialize the Threejs Engine and Scene on the provided HTML Canvas element.
...
3.  **Applying Transformations**: Implement the logic to apply state changes to the constructed Three.js mesh within the render loop.
...
### Phase 4: Testing
1.  Write unit tests with Vitest for non-rendering logic (e.g., `CubeState.js`, utility functions in `CubeController.js` and `ThreejsCubeScene.js` if mockable).
2.  Ensure separation of concerns so that business rules can be tested independently of the Threejs engine where possible.

### Phase 5: Refine and Polish
1.  Ensure SOLID principles are followed. Extract logic into small, manageable classes or functions.
2.  Implement clean code practices, meaningful variable names, and adequate comment documentation for complex 3D math/Threejs API usages.
