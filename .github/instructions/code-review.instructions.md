...
<!--
  ...
  Date: 2026-09-06T19:42:48+02:00
-->
...
## Review Scoring Rubric (Required Output)

In addition to listing individual findings, every review of files under this
instruction's `applyTo` scope must score how well the changed code follows
each chapter below, using a **1–10** scale:

- **1–2 (Critical):** Severe, widespread violations of this chapter's rules;
  code is unsafe, broken, or ignores the chapter entirely.
- **3–4 (Poor):** Multiple significant violations that harm reliability,
  maintainability, or testability.
- **5–6 (Fair):** Some violations present; code mostly works but clearly
  needs improvement in this area.
- **7–8 (Good):** Only minor deviations; the chapter's guidance is followed
  in almost all cases.
- **9–10 (Excellent):** Fully compliant, exemplary code for this chapter; no
  notable issues found.

Score exactly these five chapters, independently of each other:

1. Modern JavaScript (ES2022+) Style
2. Architecture: Separate Pure Logic From Rendering (SRP)
3. Feature Checklist: Coordinate System & 3D Cube Rendering
4. Feature Checklist: Cubes Exploding
5. Vitest Testing Guidelines

If a chapter's checklist does not apply to the changed files (for example, a
change only touches `cubes-exploding` code and never the coordinate-system
files), score that chapter `-` (not applicable) instead of guessing.

At the end of every review, output a Markdown table with this exact shape:

| Chapter | Score (1-10) | Justification |
|---|---|---|
| Modern JavaScript (ES2022+) Style | _ | _ |
| Architecture: SRP & Lifecycle | _ | _ |
| Coordinate System & 3D Cube Rendering | _ | _ |
| Cubes Exploding | _ | _ |
| Vitest Testing Guidelines | _ | _ |
| **Overall Average** | _ | _ |

- Justification must cite the specific rule(s) from this file that were, or
  were not, met — never a vague statement like "looks fine" or "needs work".
- Compute **Overall Average** as the mean of the applicable (non `-`) chapter
